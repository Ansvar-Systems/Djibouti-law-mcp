/**
 * get_djiboutian_implementations — Find Djiboutian statutes implementing a specific EU directive/regulation
...
export interface GetDjiboutianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface DjiboutianImplementationResult {
  document_id: string;
...
export async function getDjiboutianImplementations(
  db: InstanceType<typeof Database>,
  input: GetDjiboutianImplementationsInput,
): Promise<ToolResponse<DjiboutianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as DjiboutianImplementationResult[];
