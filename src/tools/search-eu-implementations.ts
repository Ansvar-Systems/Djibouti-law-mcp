/**
 * search_eu_implementations — Search EU directives/regulations that have Djiboutian implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_djiboutian_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  djiboutian_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as djiboutian_statute_count
  ...
  if (input.has_djiboutian_implementation) {
    sql += ' HAVING djiboutian_statute_count > 0';
  }
